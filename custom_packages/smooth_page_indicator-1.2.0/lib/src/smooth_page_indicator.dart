...
class SmoothPageIndicator extends StatefulWidget {
  ...
  /// Offset of page controller provided directly without need for pageController
  final double? offset;

  /// Default constructor
  const SmoothPageIndicator({
    Key? key,
    required PageController this.controller,
    required this.count,
  ...
  /// provide offset directly
  const SmoothPageIndicator.offset({
    Key? key,
    required double this.offset,
    required this.count,
    this.axisDirection = Axis.horizontal,
    this.textDirection,
    this.onDotClicked,
...
class _SmoothPageIndicatorState extends State<SmoothPageIndicator> with _SizeAndRotationCalculatorMixin {
  ...
  Widget _buildChild() => SmoothIndicator(
        offset: _offset,
        count: count,
        effect: effect,
  ...
  double get _offset {
    ...
    try {
      var offset = widget.controller?.page ?? widget.controller?.initialPage.toDouble() ?? 0;
      return offset % widget.count;
    } catch (_) {
      return widget.controller?.initialPage.toDouble() ?? 0;
    }
  }
...
class _AnimatedSmoothIndicatorState extends AnimatedWidgetBaseState<AnimatedSmoothIndicator> with _SizeAndRotationCalculatorMixin {
